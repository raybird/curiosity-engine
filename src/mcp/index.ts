...
export function getCuriosityTools(engine: CuriosityEngine): MCPTool[] {
  return [
    {
...
export async function executeTool(
  ...
  switch (toolName) {
    ...
    case "curiosity_get_report":
      if (args.type === "daily") return engine.generateDailyReport();
      if (args.type === "weekly") return engine.generateWeeklyReport();
      if (args.keyword) return engine.generateKeywordReport(args.keyword);
